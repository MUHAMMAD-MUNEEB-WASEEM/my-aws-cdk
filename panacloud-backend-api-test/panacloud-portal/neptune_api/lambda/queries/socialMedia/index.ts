...
export const getEntityFollowers = async (g: gremlin.process.GraphTraversalSource<gremlin.process.GraphTraversal>, data: GetEntityFollowersInput, cognitoUsername: string) => {
    ...
    const entities = await g.V(entityId).in_(edge.follows.L).range(offset, pageNumber * limit).choose(__.hasLabel(vertex.user.L),
        ...
        __.project("__typename", "id", "firstName","lastName", 'city', 'country', "picture_url", "total_followers", 'profileStatus','following')
            ...
            .by(__.choose(__.in_(edge.follows.L).hasId(entityId),__.constant(true),__.constant(false)))

        ,
        __.project("__typename", "id", "name", 'city', 'country', "picture_url", "owner", "total_followers", "profileStatus",'following')
