Feature: SOCIAL MEDIA MUTATIONS
  ...
  Background:
    ...
    * def entity = {city: '#? _ == "karachi" || _ == ""', country: '#? _ == "pakistan" || _ == ""', id: '#(jsonData.entity.id)', picture_url: "", profileStatus: '#? _ == "UNPUBLISHED" || _ == "PUBLISHED"', total_followers: "#number"}
    * def followEntityId = "tanzeel"
...
* text mutation = 
  ...
  And request { query: '#(mutation)' , variables:{ input : {entityId:'#(entity.id)' , followEntityId: '#(followEntityId)'}}}
  When method POST
  ...
  * print data
  # * match data..followEntifollowEntityty == followEntity
...
"""mutation unFollowEntity($input:unFollowEntityInput!) {
  ...
  * def entity = {city: '#? _ == "karachi" || _ == ""', country: '#? _ == "pakistan" || _ == ""', id: '#(jsonData.entity.id)', picture_url: "", profileStatus: '#? _ == "UNPUBLISHED" || _ == "PUBLISHED"', total_followers: "#number"}
  ...
  And request { query: '#(mutation)' , variables:{ input : {entityId:'#(entity.id)' , unFollowEntityId: '#(followEntityId)'}}}
  When method POST
...
"""mutation publishSocialMediaPost($input:SocialMediaPostInput!) {
  ...
  And request { query: '#(mutation)' , variables: { input : { by: '#(jsonData.entity.id)' , text: '#(jsonData.post.text)' , imageUrl: '#(jsonData.post.imageUrl)'}}}
  When method POST
  * match karate.range(200, 210) contains responseStatus
  * print response
  * match responseType == 'json'  
  ...
  * print data
  * def resultObj = { id : '#uuid' , createdAt:"#string" , imageUrl:'#(jsonData.post.imageUrl)',text : '#(jsonData.post.text)' , numOfLikes:"#number" , latestComments:'#[]' , by : {id:'#(jsonData.entity.id)' , picture_url:"#string"} , totalComments:"#number"}
